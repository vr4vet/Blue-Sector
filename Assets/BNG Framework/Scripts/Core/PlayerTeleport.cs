...
namespace BNG
{
    ...
    public class PlayerTeleport : MonoBehaviour
    {
        ...
        // Check status of teleportation anchors being hit
        private GameObject _anchorHit = null;
        public UnityEvent<GameObject> AnchorHitEvent = new UnityEvent<GameObject>();
        ...
        protected virtual void calculateParabola()
        {
            ...
            for (int i = 1; i < SegmentCount; i++)
            {
                ...
                if (Physics.Raycast(segments[i - 1], segVelocity, out hit, SegmentScale, CollisionLayers))
                {
                    ...
                    if (_hitObject.gameObject.CompareTag("TeleportationAnchor"))
                    {
                        _anchorHit = _hitObject.gameObject;
                        AnchorHitEvent?.Invoke(_anchorHit);
                    }
                    else if (_anchorHit)
                    {
                        _anchorHit = null;
